...
export const routes: Routes = [
  ...
  { path: 'login', loadComponent: () => import('./features/login').then((m) => m.LoginComponent) },
  {
    ...
    children: [
      ...
      { path: 'attendees', canActivate: [roleGuard('Admin', 'Organizer')], loadComponent: () => import('./features/dashboard/attendees').then((m) => m.AttendeesComponent) },
      { path: 'feedbacks', canActivate: [roleGuard('Admin', 'Organizer')], loadComponent: () => import('./features/dashboard/feedbacks').then((m) => m.FeedbacksComponent) },
      { path: 'notifications', loadComponent: () => import('./features/dashboard/notifications').then((m) => m.NotificationsComponent) },
      { path: 'audit-logs', canActivate: [roleGuard('Admin', 'Organizer')], loadComponent: () => import('./features/dashboard/audit-logs').then((m) => m.AuditLogsComponent) },
      { path: 'settings', loadComponent: () => import('./features/dashboard/settings').then((m) => m.SettingsComponent) },
    ],
  },
  { path: '**', redirectTo: '' },
];
